    ...
    return (
        <div className="space-y-12 py-8 flex flex-col items-center">
            <div className="text-center mb-8">
                <h3 className="text-4xl font-black uppercase tracking-[0.15em] text-sor7ed-yellow mb-2">Focus Engine</h3>
            ...
            <div className="relative group">
                ...
                <div className={`stealth-card w-80 h-80 rounded-full flex flex-col items-center justify-center border-4 transition-colors duration-1000 ${isWork ? 'border-sor7ed-yellow/20' : 'border-blue-500/20 shadow-[0_0_50px_rgba(59,130,246,0.1)]'}`}>
                    ...
                    <div className={`text-7xl font-black tracking-[0.15em] tabular-nums ${isWork ? 'text-white' : 'text-blue-400 animate-pulse'}`}>
                        {formatTime(timeLeft)}
                    </div>
                    <div className="mt-8 flex space-x-4">
                        <button onClick={toggleTimer} className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-[0.15em] transition-all ${isActive ? 'bg-zinc-800 text-zinc-400' : 'bg-sor7ed-yellow text-black hover:scale-105'}`}>
                            {isActive ? 'Pause' : 'Start'}
                        </button>
                        <button onClick={resetTimer} className="px-6 py-2 rounded-full border border-white/10 text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-500 hover:text-white transition-colors">
                            Reset
                        </button>
                    </div>
                </div>
                ...
                {!isWork && (
                    <div className="absolute -bottom-20 left-1/2 -translate-x-1/2 w-full text-center animate-in slide-in-from-top duration-700">
                        <div className="stealth-card p-4 border-blue-500/30 bg-blue-500/5">
                            <h4 className="text-blue-400 font-bold uppercase text-[10px] tracking-[0.15em] mb-1">Break Active</h4>
